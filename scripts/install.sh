  ...
  if [[ -d "$TARGET_DIR" ]]; then
    if [[ $FORCE -eq 1 ]]; then
      if [[ -f "${TARGET_DIR}/docker-compose.yml" ]]; then
        log "Arrêt de la pile existante avant réinstallation (--force)."
        set +e
        ( cd "$TARGET_DIR" && docker compose down --remove-orphans )
        local down_status=$?
        set -e
        if [[ $down_status -ne 0 ]]; then
          log "Avertissement : l'arrêt de la pile Docker a échoué, poursuite de l'installation."
        fi
      fi
